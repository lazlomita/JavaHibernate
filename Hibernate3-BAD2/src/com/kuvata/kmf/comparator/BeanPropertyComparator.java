package com.kuvata.kmf.comparator;
...
public class BeanPropertyComparator implements Comparator {
  ...
  public int compare(Object o1, Object o2) throws IllegalArgumentException {
	...
	Object p2 = null;
	if( o1 != null ){
		p1 = BeanPropertyUtil.getProperty(property,o1);
	}
  	if( o2 != null ){
		p2 = BeanPropertyUtil.getProperty(property,o2);
  	}
		
    if (comparator == null) 
    {
    	// JSR 12/15/04 - First try to compare strings
    	if( (p1 instanceof String) && (p2 instanceof String) )
    	{
			// Ignore case
    		String s1 = String.valueOf(p1).toUpperCase(); // calls toString safely
    		String s2 = String.valueOf(p2).toUpperCase(); 
    		return s1.compareToIgnoreCase(s2); // String implements comparable
    	}
    	else if ( p1 == null && p2 instanceof String )
    	{
    		return 1;
    	}
    	else if ( p1 instanceof String && p2 == null )
    	{
    		return -1;
    	}
    	else if ( p1 == null && p2 instanceof Date )
    	{
    		return -1;
    	}
    	else if ( p1 instanceof Date && p2 == null )
    	{
    		return 1;
    	}		
		/*
		 * If we're comparing a Timestamp to a Date, java 1.5 throws
		 * a ClassCastException. Therefore must explicitly cast the 
		 * Timestamp to a date. (Implementation inheritance vs. Type inheritance).
		 * Supposedly fixed in java 1.6.
		 */
    	else if ( p1 instanceof Timestamp && p2 instanceof Date )
		{
			Timestamp t = (Timestamp)p1;
			p1 = new Date( t.getTime() );
