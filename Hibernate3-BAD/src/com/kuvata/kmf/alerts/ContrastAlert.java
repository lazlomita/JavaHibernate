 ...
 * @author jrandesi
 */
public class ContrastAlert extends AlertDefinition 
{		
	private String desiredContrast = "";
	
	public ContrastAlert(String schemaName, Long alertId, String alertType, String alertName, String[] args, Integer frequency)
	{
		super( schemaName, alertId, alertType, alertName, args, frequency );
		
		// If the correct number of parameters were passed in -- use them
		if( args.length == 1 ) {
			this.desiredContrast = args[0];
		}		
	}
	...
	{
		SchemaDirectory.initialize( this.schemaName, "ContrastAlert", "Contrast Alert", false, false);
		try
		{		
			...
			if( alert != null )
			{
				 ...
				 * for any mcm's that do not have it's contrast value set to the specified contrast (as defined by the alert)
				 */
				...
				String hql = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
					+"WHERE mcmHistoryEntry.contrast IS NOT NULL AND mcmHistoryEntry.contrast != :contrast "
				...
				Query q = session.createQuery( hql )
					.setParameter("contrast", this.desiredContrast );
				if( alert.alertSpecificDevices() ){
					...
					if( this.addDevice( mcmHistoryEntry.getMcm().getDevice(), alert ) )
					{
						...
						if( alert.getMinimumTime() != null )
						{
							...
							String hql2 = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
								+ "WHERE mcmHistoryEntry IS NOT NULL AND mcmHistoryEntry.contrast = :param1 " 
							...
							List<McmHistoryEntry> l2 = session.createQuery( hql2 )
								.setParameter("param1", this.desiredContrast)
	...
	private void sendAlert(Alert alert, HashMap<Long, McmHistoryEntry> mcmHistoryEntries, String emailAddresses) throws InterruptedException
	{
		...
		String subject = alert.getAlertName() +" - Contrast Alert";		
		colHeaders.add( "Date" ); 
		colHeaders.add( "Device Name (MAC Address - MCM Name)" ); 
		colHeaders.add( "Current Contrast" );
		...
		for( Iterator<McmHistoryEntry> i=mcmHistoryEntries.values().iterator(); i.hasNext(); ) 
		{
			...
			String contrast = mcmHistoryEntry.getContrast() != null ? mcmHistoryEntry.getContrast() : "";
			...
			boolean valueHasChanged = valueHasChanged( alert, mcmHistoryEntry.getMcm().getMcmId(), contrast );
			...
			if( valueHasChanged )
			{
				...
				if( alert.getAttemptToFix() != null && alert.getAttemptToFix().booleanValue() == true )
				{
					...
					mcmHistoryEntry.getMcm().addMcmCommand( McmCommandType.SET_CONTRAST, this.desiredContrast, false, false );
					fixAttempted = true;
				}				
			}
			...
			rowData.add( contrast );
			allData.add( rowData );		
			lastResults.put( mcmHistoryEntry.getMcm().getMcmId(), new Object[]{contrast, rowData} );						
		}	
		...
		{		
			...
			msg.append("\n\nOne or more of the displays attached to the following device(s) are not set to the correct contrast level ("+ this.desiredContrast +")");
			if( alert.getMinimumTime() != null ){
