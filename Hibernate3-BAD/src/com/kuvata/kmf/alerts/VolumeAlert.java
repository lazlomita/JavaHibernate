 ...
 * @author jrandesi
 */
public class VolumeAlert extends AlertDefinition 
{		
	private String desiredVolume = "";

	public VolumeAlert(String schemaName, Long alertId, String alertType, String alertName, String[] args, Integer frequency)
	{
		super( schemaName, alertId, alertType, alertName, args, frequency );
		
		// If the correct number of parameters were passed in -- use them
		if( args.length == 1 ) {
			this.desiredVolume = args[0];
		}
	}
	...
	{
		SchemaDirectory.initialize( this.schemaName, "VolumeAlert", "Volume Alert", false, false);
		try
		{	
			...
			if( alert != null )
			{
				...
				String hql = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
					+"WHERE mcmHistoryEntry.volume IS NOT NULL AND mcmHistoryEntry.volume != :volume "
				...
				Query q = session.createQuery( hql )
					.setParameter("volume", this.desiredVolume );
				if( alert.alertSpecificDevices() ){
					...
					if( this.addDevice( mcmHistoryEntry.getMcm().getDevice(), alert ) )
					{
						...
						if( alert.getMinimumTime() != null )
						{
							...
							String hql2 = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
								+ "WHERE mcmHistoryEntry IS NOT NULL AND mcmHistoryEntry.volume = :param1 " 
							...
							List<McmHistoryEntry> l2 = session.createQuery( hql2 )
								.setParameter("param1", this.desiredVolume)
	...
	private void sendAlert(Alert alert, HashMap<Long, McmHistoryEntry> mcmHistoryEntries, String emailAddresses) throws InterruptedException
	{
		...
		String subject = alert.getAlertName() +" - Volume Alert";		
		colHeaders.add( "Date" ); 
		colHeaders.add( "Device Name (MAC Address - MCM Name)" ); 
		colHeaders.add( "Current Volume" );
		...
		for( Iterator<McmHistoryEntry> i=mcmHistoryEntries.values().iterator(); i.hasNext(); ) 
		{
			...
			String volume = mcmHistoryEntry.getVolume() != null ? mcmHistoryEntry.getVolume() : "";
			...
			boolean valueHasChanged = valueHasChanged( alert, mcmHistoryEntry.getMcm().getMcmId(), volume );
			...
			if( valueHasChanged )
			{
				...
				if( alert.getAttemptToFix() != null && alert.getAttemptToFix().booleanValue() == true )
				{
					...
					mcmHistoryEntry.getMcm().addMcmCommand( McmCommandType.SET_VOLUME, this.desiredVolume, false, false );
					fixAttempted = true;
				}							
			}
			...
			rowData.add( volume );
			allData.add( rowData );		
			lastResults.put( mcmHistoryEntry.getMcm().getMcmId(), new Object[]{volume, rowData} );
		}	
		...
		{		
			...
			msg.append("\n\nOne or more displays attached to the following device(s) are not set to a valid volume ("+ this.desiredVolume +")");
			if( alert.getMinimumTime() != null ){
