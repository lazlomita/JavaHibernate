 ...
 * @author jrandesi
 */
public class BrightnessAlert extends AlertDefinition 
{		
	private String desiredBrightness = "";	
	
	public BrightnessAlert(String schemaName, Long alertId, String alertType, String alertName, String[] args, Integer frequency)
	{
		super( schemaName, alertId, alertType, alertName, args, frequency );
		
		// If the correct number of parameters were passed in -- use them
		if( args.length == 1 ) {
			this.desiredBrightness = args[0];
		}		
	}
	...
	{
		SchemaDirectory.initialize( this.schemaName, "BrightnessAlert", "Brightness Alert", false, false);
		try
		{			
			...
			if( alert != null )
			{
				...
				String hql = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
					+"WHERE mcmHistoryEntry.brightness IS NOT NULL AND mcmHistoryEntry.brightness != :brightness "
				...
				hql += "AND mcmHistoryEntry.mcm.device.contentUpdateType = 'Network' "
					...
					+ "AND mcmHistoryEntry.mcm.peripheralType = '"+ PeripheralType.DISPLAY +"' "
				...
				Query q = session.createQuery( hql ).setParameter("brightness", this.desiredBrightness );
				if( alert.alertSpecificDevices() ){
					q.setParameter("alertId", alertId );
				}		
				
				List<McmHistoryEntry> l = q.list();
				LinkedHashMap<Long, McmHistoryEntry> mcmHistoryEntries = new LinkedHashMap<Long, McmHistoryEntry>();
					...
					if( this.addDevice( mcmHistoryEntry.getMcm().getDevice(), alert ) )
					{
						...
						if( alert.getMinimumTime() != null )
						{
							...
							String hql2 = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
								+ "WHERE mcmHistoryEntry IS NOT NULL AND mcmHistoryEntry.brightness = :param1 " 
							...
							List<McmHistoryEntry> l2 = session.createQuery( hql2 )
								.setParameter("param1", this.desiredBrightness)
	...
	private void sendAlert(Alert alert, HashMap<Long, McmHistoryEntry> mcmHistoryEntries, String emailAddresses) throws InterruptedException
	{
		...
		String subject = alert.getAlertName() +" - Brightness Alert";		
		colHeaders.add( "Date" ); 
		colHeaders.add( "Device Name (MAC Address - MCM Name)" ); 
		colHeaders.add( "Current Brightness" );
		...
		for( Iterator<McmHistoryEntry> i=mcmHistoryEntries.values().iterator(); i.hasNext(); ) 
		{
			...
			String brightness = mcmHistoryEntry.getBrightness() != null ? mcmHistoryEntry.getBrightness() : "";
			...
			boolean valueHasChanged = valueHasChanged( alert, mcmHistoryEntry.getMcm().getMcmId(), brightness );
			...
			if( valueHasChanged )
			{
				...
				if( alert.getAttemptToFix() != null && alert.getAttemptToFix().booleanValue() == true )
				{
					...
					mcmHistoryEntry.getMcm().addMcmCommand( McmCommandType.SET_BRIGHTNESS, this.desiredBrightness, false, false );
					fixAttempted = true;
				}
			}
			...
			rowData.add( brightness );
			allData.add( rowData );		
			lastResults.put( mcmHistoryEntry.getMcm().getMcmId(), new Object[]{brightness, rowData} );				
		}	
		...
		{							
			...
			msg.append("\n\nOne or more of the displays attached to the following device(s) are not set to the correct brightness level ("+ this.desiredBrightness +")");
			if( alert.getMinimumTime() != null ){
