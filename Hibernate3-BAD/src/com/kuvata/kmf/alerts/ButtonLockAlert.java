 ...
 * @author jrandesi
 */
public class ButtonLockAlert extends AlertDefinition 
{
	private String desiredStatus = "";
		
	public ButtonLockAlert(String schemaName, Long alertId, String alertType, String alertName, String[] args, Integer frequency)
	{
		super( schemaName, alertId, alertType, alertName, args, frequency );
		
		// If the correct number of parameters were passed in -- use them
		if( args.length == 1 ) {
			this.desiredStatus = args[0];
		}
	}
	...
	{
		SchemaDirectory.initialize( this.schemaName, "ButtonLockAlert", "Button Lock Alert", false, false);
		try
		{		
			...
			if( alert != null )
			{
				 ...
				 * and it meets the criteria based on the desiredStatus
				 */
				String hql = "";
				String criteria = "";
				...
				// If the specified desiredStatus is blank -- search where IS NULL
				if( this.desiredStatus == null || this.desiredStatus.trim().length() == 0 ){
					criteria = " IS NULL";
				}else{
					criteria = "="+ this.desiredStatus;
				}
				hql = "SELECT mcmHistoryEntry FROM McmHistoryEntry mcmHistoryEntry "
					+"WHERE mcmHistoryEntry.buttonLock "+ criteria +" "
				...
				{					
					...
					if( this.addDevice( mcmHistoryEntry.getMcm().getDevice(), alert ) )
					{
						...
						if( alert.getMinimumTime() != null )
						{
							...
							// Get the next most recent mcmHistoryEntry where buttonLock is not desiredStatus
							// If the specified power is blank -- search where IS NOT NULL
							if( this.desiredStatus == null || this.desiredStatus.trim().length() == 0 ){
								criteria = " IS NOT NULL";
							}else if(this.desiredStatus.equals("0")){
								criteria = "IS NOT NULL AND mcmHistoryEntry.buttonLock = '1' ";
							}else if(this.desiredStatus.equals("1")){
								criteria = "IS NULL OR mcmHistoryEntry.buttonLock = '0' ";
							}
							...
							if( l2.size() > 0 ){
								...
								if(l3.size() > 0){
									...
									if( this.desiredStatus == null || this.desiredStatus.trim().length() == 0 ){
										...
										isValidDevice = lastEntry == null || lastEntry.getButtonLock().equals("0");
									}else if(this.desiredStatus.equals("1")){
										isValidDevice = lastEntry != null && lastEntry.getButtonLock().equals("1");
									}
								}
							}
						}						
	...
	private void sendAlert(Alert alert, HashMap<Long, McmHistoryEntry> mcmHistoryEntries, String emailAddresses) throws InterruptedException
	{
		...
		String subject = alert.getAlertName() +" - Button Lock Alert";		
		colHeaders.add( "Date" ); 
		colHeaders.add( "Device Name (MAC Address - MCM Name)" ); 
		colHeaders.add( "Current Button Lock" );
		...
		for( Iterator<McmHistoryEntry> i=mcmHistoryEntries.values().iterator(); i.hasNext(); ) 
		{
			...
			String buttonLock = mcmHistoryEntry.getButtonLock() != null ? mcmHistoryEntry.getButtonLock() : "";
			...
			boolean valueHasChanged = valueHasChanged( alert, mcmHistoryEntry.getMcm().getMcmId(), buttonLock );
			...
			if( valueHasChanged )
			{
				...
				if( alert.getAttemptToFix() != null && alert.getAttemptToFix().booleanValue() == true )
				{
					...
					if( this.desiredStatus != null ){
						if( this.desiredStatus.equals("0") )
						{
							// Send an mcm command to try to power on the monitor
							mcmHistoryEntry.getMcm().addMcmCommand( McmCommandType.BUTTON_LOCK_ON, null, false, false );
							fixAttempted = true;
						}
						...
						else if( this.desiredStatus.equals("1") )
						{
							...
							mcmHistoryEntry.getMcm().addMcmCommand( McmCommandType.BUTTON_LOCK_OFF, null, false, false );
							fixAttempted = true;
						}	
					}
				}				
			...
			rowData.add( buttonLock );
			allData.add( rowData );		
			lastResults.put( mcmHistoryEntry.getMcm().getMcmId(), new Object[]{buttonLock, rowData} );
		}	
		...
		{		
			...
			msg.append("\n\nOne or more displays attached to the following device(s) are set to the button lock status of '"+ this.desiredStatus +"'");
			if( alert.getMinimumTime() != null ){
