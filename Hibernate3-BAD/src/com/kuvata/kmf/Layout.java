...
public class Layout extends Entity implements Historizable {
	 ...
	 */
	public static int searchLayoutsCount(AttrDefinition attrDefinition, String layoutNameSearchString, String selectedSearchOption, String searchString, 
			String[] selectedSearchOptions, String minDate, String maxDate, String minNumber, String maxNumber, int startingRecord) throws ParseException
	{
		int result = 0;
		String hql = buildSearchHql( layoutNameSearchString, selectedSearchOption, searchString, selectedSearchOptions, minDate, maxDate, minNumber, maxNumber, true );		
	...
	/**
	 * Returns a page of layouts according to the given search criteria.
	 * 
	 * @param attrDefinition
	 * @param assetNameSearchString
	 * @param selectedSearchOption
	 ...
	 * @throws ParseException
	 */
	public static EntityResultsPage searchLayouts(AttrDefinition attrDefinition, String layoutNameSearchString,  
			...
			String maxNumber, int startingRecord, int selectedItemsPerPage) throws ParseException
	{
		EntityResultsPage result = null;
		int pageNum = startingRecord / selectedItemsPerPage;
		String hql = buildSearchHql( layoutNameSearchString, selectedSearchOption, searchString, selectedSearchOptions, minDate, maxDate, minNumber, maxNumber, false );
		...
		{			
					...
					&& ((minDate != null && minDate.length() > 0) && (maxDate != null && maxDate.length() > 0)) )
			{
				...
				SimpleDateFormat df = new SimpleDateFormat( Constants.DATE_TIME_FORMAT_DISPLAYABLE );
				Date[] params = new Date[]{ df.parse( minDate ), df.parse( maxDate ) }; 
				result = Layout.getLayouts( hql, pageNum, selectedItemsPerPage, params );	
			}
			// If this is a multi-select attrDefinition
			else if( attrDefinition != null 
					&& attrDefinition.getSearchInterface().getPersistentValue().equalsIgnoreCase( SearchInterfaceType.MULTI_SELECT.getPersistentValue() ) 
			...
			{
				// Use the selectedSearchOptions a named parameter in the query object 
				result = Layout.getLayouts( hql, pageNum, selectedItemsPerPage, selectedSearchOptions, "attrValues" );
			}else{
				result = Layout.getLayouts( hql, pageNum, selectedItemsPerPage );
			}
		}
		// Date query
		else if( attrDefinition.getType().getPersistentValue().equalsIgnoreCase( AttrType.DATE.getPersistentValue() ) )
	 ...
	 */
	public static EntityResultsPage getLayouts(String hql, int pageNum, int iSelectedItemsPerPage, Object[] params, String namedParameter) throws HibernateException 
	{
		EntityResultsPage result = null;		
		Session session = HibernateSession.currentSession();		
		...
		result = new EntityResultsPage(q, pageNum, iSelectedItemsPerPage);				
		return result;
	}
	 ...
	 * @return Returns a page of devices
	 */
	public static EntityResultsPage getLayouts(String hql, int pageNum, int iSelectedItemsPerPage) throws HibernateException 
	{		
		return getLayouts( hql, pageNum, iSelectedItemsPerPage, null );
	}	
	
	/**
	 * @return Returns a page of devices
	 */
	public static EntityResultsPage getLayouts(String hql, int pageNum, int iSelectedItemsPerPage, Object[] params) throws HibernateException 
	{		
		return getLayouts( hql, pageNum, iSelectedItemsPerPage, params, null );
	}		
	...
	{				
		...
		newLayout.copyPermissionEntries( this );		
