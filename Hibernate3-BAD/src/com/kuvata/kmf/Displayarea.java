...
public class Displayarea extends Entity implements Historizable { 
	...
	/**
	 * Returns a page of displayareas according to the given search criteria.
	 * 
	 * @param attrDefinition
	 * @param assetNameSearchString
	 * @param selectedSearchOption
	 ...
	 * @throws ParseException
	 */
	public static EntityResultsPage searchDisplayareas(AttrDefinition attrDefinition, String displayareaNameSearchString,  
			...
			String maxNumber, int startingRecord, int selectedItemsPerPage, boolean getShared) throws ParseException
	{
		EntityResultsPage result = null;
		int pageNum = startingRecord / selectedItemsPerPage;
		String hql = buildSearchHql( displayareaNameSearchString, selectedSearchOption, searchString, selectedSearchOptions, minDate, maxDate, minNumber, maxNumber, getShared, false );
		...
		{			
					...
					&& ((minDate != null && minDate.length() > 0) && (maxDate != null && maxDate.length() > 0)) )
			{
				...
				SimpleDateFormat df = new SimpleDateFormat( Constants.DATE_TIME_FORMAT_DISPLAYABLE );
				Date[] params = new Date[]{ df.parse( minDate ), df.parse( maxDate ) }; 
				result = Displayarea.getDisplayareas( hql, pageNum, selectedItemsPerPage, params );
			}
			// If this is a multi-select attrDefinition
			else if( attrDefinition != null 
					&& attrDefinition.getSearchInterface().getPersistentValue().equalsIgnoreCase( SearchInterfaceType.MULTI_SELECT.getPersistentValue() ) 
			...
			{
				// Use the selectedSearchOptions a named parameter in the query object 
				result = Displayarea.getDisplayareas( hql, pageNum, selectedItemsPerPage, selectedSearchOptions, "attrValues" );
			}else{
				result = Displayarea.getDisplayareas( hql, pageNum, selectedItemsPerPage );
			}
		}
		// Date query
		else if( attrDefinition.getType().getPersistentValue().equalsIgnoreCase( AttrType.DATE.getPersistentValue() ) )
	 ...
	 */
	public static EntityResultsPage getDisplayareas(String hql, int pageNum, int iSelectedItemsPerPage, Object[] params, String namedParameter) throws HibernateException 
	{
		EntityResultsPage result = null;		
		Session session = HibernateSession.currentSession();		
	 ...
	 */
	public static EntityResultsPage getDisplayareas(String hql, int pageNum, int iSelectedItemsPerPage) throws HibernateException 
	{		
		return getDisplayareas( hql, pageNum, iSelectedItemsPerPage, null );
	}	
	
	/**
	 * @return Returns a page of devices
	 */
	public static EntityResultsPage getDisplayareas(String hql, int pageNum, int iSelectedItemsPerPage, Object[] params) throws HibernateException 
	{		
		return getDisplayareas( hql, pageNum, iSelectedItemsPerPage, params, null );
	}		
