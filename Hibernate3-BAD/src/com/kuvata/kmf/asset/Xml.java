...
public class Xml extends Asset implements IXml
{
	...
	public static Xml create(HttpServletRequest request, HttpServletResponse response, String assetName, Float length, 
			Displayarea da, Layout l, boolean useFileLocFromServer, String fileLocation, FormFile formFile, boolean waitForFileUpload) throws IOException, InterruptedException
	{
		...
		{				
			// Upload the file
			fileLoc = uploadFile( request, response, assetId, a.getAssetType(), formFile );
			File newFile = new File( fileLoc );
			if( newFile != null )
			{
				// Rename the file using its adler32
				Long adler32 = Asset.calculateAdler32( fileLoc );
				String newFileLoc = Asset.buildFileloc( fileLoc, adler32 );
				Long filesize = newFile.length();
				Files.renameTo( newFile, new File(newFileLoc) );
				fileLoc = newFileLoc;
				a.setAdler32( adler32 );
				a.setFilesize( filesize );
			}			
			a.setFileloc( fileLoc );
			a.createThumbnail( Asset.MAX_DIMENSION );
			a.update();		
		}			
		return a;
	}
	
	public void update(HttpServletRequest request, HttpServletResponse response, String assetName, Float length, 
			Displayarea da, Layout l, boolean useFileLocFromServer, String fileLocation, FormFile formFile, boolean waitForFileUpload) throws IOException, InterruptedException
	{
		...
		else
		{
			// If the fileloc property has changed -- re-upload the file
			if( formFile != null ) {
				if( this.getFileloc() == null || this.getFileloc().equals( formFile.getFileName() ) == false)
				{
					if( formFile.getFileName() != null && formFile.getFileName().length() > 0 )
					{
						String fileLoc = uploadFile( request, response, this.getAssetId(), this.getAssetType(), formFile );
						File newFile = new File( fileLoc );
						if( newFile != null )
						{
							// Rename the file using its adler32
