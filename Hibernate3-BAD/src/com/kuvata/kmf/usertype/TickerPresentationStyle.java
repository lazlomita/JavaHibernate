...
{	
	...
	public static ArrayList getTickerPresentationStyles()
	{
		...
		try {
			// Attempt to locate the "Ticker.isOpenGL" property
			String strIsOpenGL = KuvataConfig.getPropertyValue("Ticker.isOpenGL", false);
			isOpenGL = Boolean.valueOf( strIsOpenGL ).booleanValue();
		} catch (KmfException e) {
			logger.error("Could not locate property \"Ticker.isOpenGL\". Using default: true");
		}
