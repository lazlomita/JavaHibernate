 ...
 */
public class Grp extends Entity implements TreeViewable, Historizable {
	...
	public String treeViewFormat(int recursionLevel, boolean includeLeaves, boolean includeAllLeaves, boolean includeHref, boolean includeDoubleClick, boolean doubleClickLeavesOnly, String treeNodeCssClass, Method allBranchMethod)
	{
		...
		{				
			...
			if( recursionLevel > 0 )
			{
				// If we've recurred beyond the first level, disable the grpMember filter since we can only have one filter enabled at a time
				FilterManager.disableFilter( FilterType.GRP_MEMBER_FILTER );

				// If we are not including all the leaves
				if( includeAllLeaves == false) 
				{
					// Set the flag to retrieve the remaining child nodes
					onClick = "\'javascript:grpOnClick("+ this.getGrpId() +", true, "+ String.valueOf(includeHref) +", "+ String.valueOf(includeDoubleClick) +", "+ String.valueOf(doubleClickLeavesOnly) +")\'";
				}
			}
			result.append("[");
