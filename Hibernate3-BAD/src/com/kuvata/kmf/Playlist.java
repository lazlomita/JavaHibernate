...
import parkmedia.usertype.ContentSchedulerHookType;
...
import parkmedia.usertype.PlaylistSearchType;
import parkmedia.usertype.ScreenTransitionType;
...
import com.kuvata.kmm.PlaylistAssetsPage;
import com.kuvata.kmm.PlaylistResultsPage;

public class Playlist extends Entity implements TreeViewable, Historizable {

	private static KMFLogger logger = (KMFLogger)KMFLogger.getInstance( ImportContentRotationThread.class );
	 ...
	 */
	public Page getPlaylistAssets(String orderBy, int pageNum, int iSelectedItemsPerPage) throws HibernateException 
	{
		Page result = null;		
		Session session = HibernateSession.currentSession();
		
		// Not ideal, but need to include the pa alias in the given order by clause 
		// in order to perform UPPER() function on string comparisons
		String hql = "SELECT pa "
			+ "FROM PlaylistAsset pa "
			+ "WHERE pa.playlist.playlistId= "+ this.getPlaylistId().toString() +" "
			+ "ORDER BY "+ orderBy;		
		Query q = session.createQuery( hql );
		result = new PlaylistAssetsPage(q, pageNum, iSelectedItemsPerPage);				
		return result;
	}
