...
public class AttrDefinition extends Entity {
	...
	private static KMFLogger logger = (KMFLogger)KMFLogger.getInstance( AttrDefinition.class );
	 ...
	 */
	public static AttrDefinition addAttrDefinition(String className, String attrName, AttrType attrType, SearchInterfaceType searchToolType, Boolean adServer, Boolean showInReport) throws HibernateException, KmfException
	{							
		// Make sure an attribute with this name does not already exist for this entity
		if(attrDefinitionExists(className, attrName)){
			throw new KmfException("Cannot create attribute definition, "+ attrName +", for entity, "+ className +". Attribute already exists.");
		}
	 ...
	 * @throws Exception
	 */
	public static AttrDefinition getAttributeDefinition(Class c, String name) throws HibernateException, ClassNotFoundException, KmfException
	{		
		...
		if( adType != null )
		{
			...
			if( adType.equalsIgnoreCase( AttrType.STRING.getAttrTypeName() ) )
			{
				...
				for( Iterator j=ad.getAttrValues().iterator(); j.hasNext(); )
				{
					...
					attrValues.add( new LabelValueBean( strAttrValue, strAttrValue ) );	
				}												
			}
		}
	 ...
	 */
	public static boolean saveOrUpdateAttr(Entity e, String attrValue, String[] attrValues, AttrDefinition ad, boolean useExistingMultiSelectValues, boolean append) throws ParseException, KmfException
	{
		...
		if( ad.getType() != null )
		{
			...
			if(ad.getSearchInterface().equals(SearchInterfaceType.MULTI_SELECT)){
				if(attrValues != null){
					...
					// Append errors to logs
					if(errors.length() > 0){
						throw new KmfException(errors);
					}
				}
			}
	...
	public static boolean saveOrUpdateAttr(Entity e, String attrValue, String[] attrValues, String userInterface, AttrDefinition ad, Attr attr, boolean useExistingMultiSelectValues, boolean append) throws ParseException, KmfException
	{
		DateFormat dateTimeFormat = new SimpleDateFormat( Constants.DATE_TIME_FORMAT_DISPLAYABLE);
		boolean valueHasChanged = false;
